...
{
    EUSART2_Write(value);       //     "请将MCU串口发送函数填入该函数-串口发送函数  
...
void all_data_update(void)
{
    ...
    mcu_dp_value_update(DPID_TEMP_CURRENT,wifi_sht30_data[0]);          //VALUE型数据上报;当前当前温度
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,wifi_sht30_data[2]);        //VALUE型数据上报;当前当前湿度
    mcu_dp_value_update(DPID_BATTERY_PERCENTAGE,50);                    //VALUE型数据上报;当前电池电量
    mcu_dp_value_update(DPID_TEMP_SAMPLING, Set_sample_Value);          //VALUE型数据上报;当前温度采样时间设置
    mcu_dp_bool_update(DPID_CHARGE_STATE,Charge_Flag);                  //BOOL型数据上报;当前充电状态
//    mcu_dp_enum_update(DPID_TEMP_ALARM,当前温度报警);                 //枚举型数据上报;当前温度报警
//    mcu_dp_enum_update(DPID_HUM_ALARM,当前湿度报警);                  //枚举型数据上报;当前湿度报警
